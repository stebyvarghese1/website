/* ==========================================================================
   ThreatLens — Apple Experience Simulator (simulator.js)
   Clean, responsive incident sandbox with Why? modal sheet
   ========================================================================== */
(function () {
  'use strict';
  ...
  const SCENARIOS = {
    ...
    canary_ransomware: {
      ...
      why: {
        ...
        what: 'An untrusted executable attempted unauthorized bulk file encryption and touched hidden ThreatLens canary files.',
  ...
  const scBtns       = document.querySelectorAll('.sim-tab-btn, .scenario-btn');
  ...
  function loadScenario(key) {
    ...
    if (feedStream) {
      ...
      d.events.forEach(ev => {
        ...
        row.className = 'sim-event-item';

        const badgeMap = { crit: 'badge-crit', warn: 'badge-warn', info: 'badge-info' };
        const badgeLbl = { crit: 'CRITICAL', warn: 'WARNING', info: 'INFO' };

        row.innerHTML = `
          <div class="sim-event-info">
            <span class="sim-event-badge ${badgeMap[ev.type] || 'badge-info'}">${badgeLbl[ev.type] || 'INFO'}</span>
            <span class="sim-event-msg">${ev.msg}</span>
          </div>
          <button class="sim-why-btn" data-key="${key}">Why?</button>
        `;
        feedStream.appendChild(row);

        row.querySelector('.sim-why-btn').addEventListener('click', e => {
          e.stopPropagation();
    ...
    if (lineageChain) {
      ...
      d.lineage.forEach((node, i) => {
        ...
        el.className = `lineage-node ${node.cls}`;
  ...
  function reset() {
    ...
    if (statusPill) {
      statusPill.textContent = 'ALL SENSORS SECURE';
      statusPill.style.color = '#30d158';
    }

    if (hwCpuVal) hwCpuVal.textContent = '3%';
    if (hwCpuBar) hwCpuBar.style.width = '3%';
    if (hwRamVal) hwRamVal.textContent = '21%';
    if (hwRamBar) hwRamBar.style.width = '21%';
    ...
    if (feedStream) {
      feedStream.innerHTML = `
        <div class="sim-event-item">
          <div class="sim-event-info">
            <span class="sim-event-badge badge-info" style="background:rgba(48,209,88,.15);color:#30d158">SECURE</span>
            <span class="sim-event-msg">Baseline restored. Active sentinels monitoring endpoint telemetry.</span>
          </div>
        </div>`;
    }

    if (lineageChain) {
      lineageChain.innerHTML = `<div class="lineage-node ok">System Idle · Normal Kernel Dispatch</div>`;
    }
  }
